...
def fesom_ice_area(src_path, mesh_diag_path, years=(1979, 2015), box=[-180, 180, -90, -60], aice_threshhold=.15, log=True):
    """
    ...
    # Create sea ice mask
    ds_cropped['ice_mask'] = (('time','nod2'), np.where(ds_cropped.a_ice > aice_threshhold, True, False))

    # Sum over non-masked nodal areas
    ds_cropped['sea_ice_area'] = (
    ('time'), (ds_cropped.ice_mask * mesh_diag_cropped.nod_area.isel(nz=0)).sum(dim='nod2').values)
    ...
    ds_cropped.sea_ice_area.attrs['bounding box'] = f'Longitude: {box[0]}E to {box[1]}E, Latitude: {box[2]}N to {box[3]}N'

    return ds_cropped


def fesom_ice_volume(src_path, mesh_diag_path, years=(1979, 2015), box=[-180, 180, -90, -60], log=True):
    ...
    sea_ice_volume = (mesh_diag_cropped.nod_area.isel(nz=0) * ds_mice_cropped.m_ice * ds_aice_cropped.a_ice).sum(dim='nod2')
    ...
    ds_mice_cropped.sea_ice_volume.attrs['bounding box'] = f'Longitude: {box[0]}E to {box[1]}E, Latitude: {box[2]}N to {box[3]}N'
