...
def fesom_ice_area(ds, mesh_diag, box=[-180, 180, -90, -60], aice_threshhold=.15):
    """
    ...
    inds = np.where((mesh_diag.lon > box[0]) & (mesh_diag.lon < box[1]) & (
        mesh_diag.lat > box[2]) & (mesh_diag.lat < box[3]))[0]
    ...
    # Create sea ice mask
    ds_cropped['ice_mask'] = (('time', 'nod2'), np.where(
        ds_cropped.a_ice > aice_threshhold, True, False))
    ...
    ds_cropped.sea_ice_area.attrs[
        'bounding box'] = f'Longitude: {box[0]}E to {box[1]}E, Latitude: {box[2]}N to {box[3]}N'
...
def fesom_ice_volume(ds_aice, ds_mice, mesh_diag, box=[-180, 180, -90, -60]):
    ...
    sea_ice_volume = (mesh_diag_cropped.nod_area.isel(
        nz=0) * ds_mice_cropped.m_ice * ds_aice_cropped.a_ice).sum(dim='nod2')
    ...
    ds_mice_cropped.sea_ice_volume.attrs[
        'bounding box'] = f'Longitude: {box[0]}E to {box[1]}E, Latitude: {box[2]}N to {box[3]}N'
