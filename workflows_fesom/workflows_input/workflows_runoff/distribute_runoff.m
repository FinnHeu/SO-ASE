...
%m_pcolor(xreg,yreg,runoff_raw)
%shading flat
% m_elev('contour',[-3500:500:-500]);
%m_grid; 
%m_coast;
%colormap jet
...
%runoff_test1=ncread([meshpath,'forcing_data_on_grid/runoff_2000.nc'],'runoff');
%runoff_test2=ncread([meshpath,'runoff_JRA55_monthly_2000.nc'],'runoff');


%f = figure;
%m_proj('stereographic','lat',90,'long',0,'radius',55);
%m_proj('lambert','long',[0 50],'lat',[30  50]);
%xc=xcoord(elem);
%yc=ycoord(elem);
%[XC,YC]=m_ll2xy(xc,yc);
%aux1=mean(runoff_test1(:,1:31),2);
%aux1=mean(runoff_test2(:,1),2);
%aux=aux1(elem);
%pp=patch(XC,YC,aux);
...
%caxis([0 0.5]*1.0e-4)
%cb=colorbar;
%title('Jan 2000 - JRA55')


%runoff_test=ncread([meshpath,'/forcing_data_on_grid/runoff_2000.nc'],'runoff');
%f = figure;
%m_proj('stereographic','lat',90,'long',0,'radius',55);
%xc=xcoord(elem);
%yc=ycoord(elem);
...
%caxis([0 0.1]*1.0e-3)
%cb=colorbar;
%title('Jan 2000 - Dai09')


%%% test TSS
%A=load(['black_sea_poly.poly']);
%IN_BS=inpolygon(xcoord,ycoord,A(:,2),A(:,3));
%A=load(['asow_poly.poly']);
%IN_AS=inpolygon(xcoord,ycoord,A(:,2),A(:,3));
%A=load(['med_poly.poly']);
%IN_MED=inpolygon(xcoord,ycoord,A(:,2),A(:,3));
%for k=1:12
%  aux=runoff_test(:,k)./1000;
%  ts_river_runoff_bs(k)=sum(aux.*patch_area'.*IN_BS'); %% m/s*m*m 
%  ts_river_runoff_as(k)=sum(aux.*patch_area'.*IN_AS'); %% m/s*m*m
%  ts_river_runoff_med(k)=sum(aux.*patch_area'.*IN_MED'); %% m/s*m*m
%end
%obs=[260 282 334 404 418 354 293 231 199 196 223 254 ]; % km^3/yr
%obs=obs*1.0e9/(365*24*60*60)

%mean(ts_river_runoff_bs)
%mean(ts_river_runoff_bs-ts_river_runoff_as)
%mean(obs)


%figure
%p1=plot(1:12,ts_river_runoff_bs,'b','Linewidth',2)
%hold on
%p2=plot(1:12,ts_river_runoff_bs-ts_river_runoff_as,'c','Linewidth',2)
%hold on
%p3=plot(1:12,obs,'r','Linewidth',2)
% hold on
% p4=plot(1:12,ts_river_runoff_med,'g','Linewidth',2)

%set(gca,'Xtick',1:12)
%set(gca,'XTickLabel',['Jan';'Feb';'Mar';'Apr';'May';'Jun';'Jul';'Aug';'Sep';'Oct';'Nov';'Dec'])
%ylabel('River discharge (m^3/s)')
%set(gca,'xlim',[1 12])
%legend([p1 p2 p3],'BS+Asow, Dai et al. (2009)','BS, Dai et al. (2009)','BS, Kara et al. (2007)')
%set(gca,'fontsize',16)

%figure
%p4=plot(1:12,ts_river_runoff_med+ts_river_runoff_bs,'g','Linewidth',2)
%set(gca,'Xtick',1:12)
%set(gca,'XTickLabel',['Jan';'Feb';'Mar';'Apr';'May';'Jun';'Jul';'Aug';'Sep';'Oct';'Nov';'Dec'])
%ylabel('River discharge (m^3/s)')
%set(gca,'xlim',[1 12])
...
%mean(ts_river_runoff_med+ts_river_runoff_bs)
