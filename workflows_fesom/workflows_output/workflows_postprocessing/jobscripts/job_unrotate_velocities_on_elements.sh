...
LOG_DIR=./logs/unrotate/elements/
mkdir -p "$LOG_DIR"
...
for year in $(seq $year_start $year_end); do
    ...
    ~/micromamba/envs/pyfesom2_dev/bin/python ./../python/unrotate_velocities_on_elements.py "$year" "$base_path_model" > "$LOG_DIR/unrotate_$year.log" 2>&1 &
done
