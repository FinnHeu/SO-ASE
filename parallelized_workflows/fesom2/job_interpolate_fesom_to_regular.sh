...
for year in $(seq $year_start $year_end); do
    ...
    ~/micromamba/envs/pyfesom2_dev/bin/python interpolate_fesom_to_regular.py "$year" "$base_path_model" "$base_path_mesh" "$min_lat" "$max_lat" "$min_lon" "$max_lon" "$lat_increment" "$lon_increment" "$variables_to_interpolate" > "$LOG_DIR/unrotate_$year.log" 2>&1 &
done
